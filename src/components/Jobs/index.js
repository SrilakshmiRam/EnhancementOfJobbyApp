...
class Jobs extends Component {
  ...
  getJobDetails = async () => {
    const {searchInput, checkBoxesList, salary, locationsList} = this.state
    const type = checkBoxesList.join(',')
    const locationName = locationsList.join(',')
    ...
    const url = `https://apis.ccbp.in/jobs?employment_type=${type}&minimum_package=${salary}&search=${searchInput}&location=${locationName}`
  ...
  onChangeLocation = event => {
    ...
    if (locationsList.includes(event.target.id)) {
      const updatedList = locationsList.filter(each => each !== event.target.id)
      this.setState(
        {
      ...
      this.setState(
        prevState => ({
          locationsList: [...prevState.locationsList, event.target.id],
        }),
        this.getJobDetails,
      )
    }
          ...
          <div className="employee-details-container">
            ...
            <ul className="employment-types-list">
              {employmentTypesList.map(eachItem => (
                <li
                  className="employetypes-container"
            ...
            <ul className="locations-list">
              {locationsList.map(eachItem => (
                ...
                >
                  ...
                  />
                  <label htmlFor={eachItem.locationId} className="label-input">
                    {eachItem.label}
                  </label>
                </li>
              ))}
